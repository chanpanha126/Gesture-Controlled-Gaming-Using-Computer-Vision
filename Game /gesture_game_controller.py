...
# Special Key Mapping for macOS/OpenCV
SPECIAL_KEYS = {
    0: "up",
    1: "down",
    2: "left",
...
print("\n--- GESTURE GAME CONTROLLER (OPTIMIZED) ---")
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if len(gesture_buffer) >= 1:
                current_gesture = max(set(gesture_buffer), key=list(gesture_buffer).count)
    ...
    # ACTIONS
    if not mapping_mode and current_gesture in gesture_to_key:
        key_to_press = gesture_to_key[current_gesture]
        current_time = time.time()
        
        if current_time - last_key_time > COOLDOWN_TIME:
            if key_to_press and key_to_press != "None":
